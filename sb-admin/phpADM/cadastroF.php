<?php 
	...
	if(isset($_FILES['imagem']) && $_FILES['imagem']['size'] > 0){
		...
		if(move_uploaded_file($imagem["tmp_name"], $path)) {
			...
			// Consulta SQL para FRIOS E LATICÍNIOS
			$query = "INSERT INTO produtos (nome, descricao, categoria, subcategoria, preco, status, path) VALUES ('$nome', '$descricao', 'Frios e Laticínios', '$subcategoria', '$preco', '$status', '$path')";
			
			$result = mysqli_query($link, $query);

			if($result){
   
    echo "<script>alert('Produto cadastrado com sucesso!'); window.location.href = '../formularios/friosF.html';</script>";
} else {
