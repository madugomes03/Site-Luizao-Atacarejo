<?php 
	// --- ATIVANDO MODO DE ERROS (Para descobrir o problema) ---
	...
	if (!isset($_POST['nome'])) {
		die("Erro: O formulário não enviou os dados básicos (nome). Verifique o HTML.");
	}
	...
	if(isset($_FILES['imagem']) && $_FILES['imagem']['size'] > 0){
		...
		if(move_uploaded_file($imagem["tmp_name"], $path)) {
			...
			// Consulta SQL
			$query = "INSERT INTO produtos (nome, descricao, categoria, subcategoria, preco, status, path) VALUES ('$nome', '$descricao', 'Açougue', '$subcategoria', '$preco', '$status', '$path')";
...
if($result){
    ...
    echo "<script>alert('Produto cadastrado com sucesso!'); window.location.href = '../formularios/acougueF.html';</script>";
} else {
			...
			echo "Falha ao mover o arquivo para a pasta '$pasta'. Verifique as permissões da pasta.";
		}
	} else {
