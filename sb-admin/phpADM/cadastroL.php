<?php 
	...
	if(isset($_FILES['imagem']) && $_FILES['imagem']['size'] > 0){
		...
		if(move_uploaded_file($imagem["tmp_name"], $path)) {
			...
			// Consulta SQL para LIMPEZA
			$query = "INSERT INTO produtos (nome, descricao, categoria, subcategoria, preco, status, path) VALUES ('$nome', '$descricao', 'Limpeza', '$subcategoria', '$preco', '$status', '$path')";
			
			$result = mysqli_query($link, $query);

			if($result){
    // Redireciona de volta para o arquivo do formulário (acougueF.html)
    echo "<script>alert('Produto cadastrado com sucesso!'); window.location.href = '../formularios/limpezaF.html';</script>";
} else {
